...
class export_excel
{
    ...
    global $TMPL;
		...
		$out .="\n";
		$out .= '"Nro. de Caso","Fecha ingreso","Hora","Cliente","Tipo de problema","Etapa de solicitud","Descripcion problema","Departamento en consulta","Otro dep. afectado","Email cliente","Complejo","Responsable Viva","Responsable GyM","Agente inspeccion","Agente arreglo"';
		$out .="\n";
		...
		while ($campo = mysql_fetch_array($result)) {
          ...
          case "11":
			...
			$out .= utf8_decode ('"'.$campo["entry_id"].'","'.$fecha.'","'.$hora.'","'.$campo["form_field_2"].'","'.$campo["form_field_6"].'","'.$txt_etapa.'","'.$campo["form_field_15"].'","'.$campo["form_field_10"].'","'.$campo["form_field_11"].'","'.$campo["form_field_3"].'","'.$campo["form_field_43"].'","'.$campo["form_field_14"].'","'.$campo["form_field_13"].'","'.$campo["form_field_30"].'","'.$campo["form_field_31"].'"');
			$out .="\n";
			// for ($i = 0; $i < $columns; $i++) {
			// $out .='"'.$l["$i"].'",';
			// }
