...
class Ajw_export {
	...
	function Ajw_export() {
		...
		$TMPL = $this->EE->TMPL;
		//echo "<br> archivo ".$filename= $TMPL->fetch_param('nombre_file');
		...
		if( $this->channel !== FALSE ) {
			...
			foreach( $query->result_array() as $row ) {
				$fields[] = "d.field_id_".$row["field_id"]." as ".$row["field_label"];
			}
		...
		if( $filename != "" ) {
			...
			if ( ! write_file($filename, $data)){echo "<br>mela"; }else{echo "<br>exito";}
